...
public static class Base64ImageProvider
{
    ...
    public static ImageSource ConvertToImageSource(string Base64Imagestring)
    {
        try
        {
    ...
    public static async Task<string> SaveImageToCache(string Base64Imagestring)
    {

        string cacheDir = Path.Combine(FileSystem.CacheDirectory, "cardimages");
        if (!Directory.Exists(cacheDir))
            Directory.CreateDirectory(cacheDir);

        string filePath = Path.Combine(cacheDir, $"{Guid.NewGuid()}.png");

        using (MemoryStream imageDecodeStream = new(Convert.FromBase64String(Base64Imagestring)))
        {
            using FileStream fileStream = File.Create(filePath);
            await imageDecodeStream.CopyToAsync(fileStream);
        }
    ...
    public static void ClearCache()
    {
        ...
        try
        {
            string[] filePaths = Directory.GetFiles(cacheDir);
